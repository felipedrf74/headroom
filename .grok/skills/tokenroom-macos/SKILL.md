...
description: >-
  Native SwiftUI rules for Tokenroom, a macOS menu-bar app that shows Grok,
...
Protocol `ProviderClient.fetch() async -> Result<QuotaSnapshot, ProviderError>`. Parallel refresh. One failure never blocks the others. Credentials are read in place and never copied into Tokenroom storage or logs. Cache is percentages, reset times, and provider names only.
